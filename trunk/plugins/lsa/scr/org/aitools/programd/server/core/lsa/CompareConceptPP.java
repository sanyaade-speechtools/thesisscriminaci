...
public class CompareConceptPP implements CompareConcept {
	...
	public CompareConceptPP(String dbUrl, String dbName, String dbUsername, String dbPwd, String dbDriver, int nsigma){
		this(dbUrl, dbName, dbUsername, dbPwd, dbDriver, nsigma, 0.5);
	}
	
	public CompareConceptPP(String dbUrl, String dbName, String dbUsername, String dbPwd, String dbDriver, int nsigma, double competence){
		
		this.competence=competence;
		databasePP = new DatabasePP(dbUrl, dbName, dbUsername, dbPwd, dbDriver);
		...
		for(int i=0;i<numConcept;i++){			
			...
			for(int j:index1){
				double hcod[]=databasePP.getCodificaDocumento(j).getCoding(databasePP);
				int relevance=databasePP.getDocumentRelevance(j);
	...
	public List<StringDouble> getQueryRelated(String request, double competence){
		...
		while(tokenizer.hasMoreElements()){
			...
			double[] word = databasePP.getCodificaParola(tokenizer.nextToken()).ctx_sinistro;
			if(word!=null){
		...
		for(int i=0; i<numConcept; i++){
			...
			double relation=OperazioneArray.prodotto(cod, concept);
